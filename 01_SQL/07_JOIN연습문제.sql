...
SELECT D.EMP_NAME, E.EMP_NAME, E.DEPT_CODE  FROM employee E
...
SELECT EMP_NAME, JOB_NAME, (SALARY+(SALARY*NVL(BONUS,0)))*12 AS 급여 FROM EMPLOYEE
LEFT JOIN JOB USING(JOB_CODE)
...
WHERE FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD'))/12)=
    (SELECT MIN(FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD'))/12)) FROM employee);
