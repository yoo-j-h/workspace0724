/*
    ...
    데이터베이스에서 사용하는 SQL명령어 중 데이터를 저장할 객체의 구조를 정의 하거나 변경하는 역할을 함,
    즉, 실제 데이커값이 아닌 규칙 자체를 정의 하는 언어
...
/*
    *UNIQUE 제약조건
    ...
    컬럼값에 중복값을 제한하는 제약조건
    삽임/수정시 기존에 있는 데이터 값중 중복값이 있을경우 오류를 발생시킴
*/
...
--> 오류구문을 제약조건명으로 알려줌
--> 쉽게 어떤 제약조건이 위반인지 알기 어려움
--> 제약조건 부여시 제약조건명을 지정해주면 오류구문에서 제약조건 확인이 쉽다.

----------------------------------------------------------------------------
/*
    제약조건명 부여시 함께 제약조건명을 부여
    >컬럼레벨방식
    CREATE TABLE 테이블명(
...
CREATE TABLE MEMBER (
    ...
    MEM_ID VARCHAR2(30)CONSTRAINT MEMID_NT NOT NULL UNIQUE,
    ...
    EMAIL VARCHAR2(50),
    CREATE_AT DATE, 
    CONSTRAINT MEMEMAIL_NT UNIQUE(EMAIL) --테이블 레벨 부여방식
);
...
/*
    *CHECK(조건식)
    ...
    데이터를 추가/수정시 조건식을 만족해야 정상적인 실행이 가능
*/
DROP TABLE MEMBER;
...
INSERT INTO MEMBER
VALUES(2, 'USER02', 'PASS02', '홍길순', '여', '010-1111-2222', 'bbb@naver.com', '25/07/30');
--CHECK 제약조건 때문에  '남', '여' 이외에 성별입력시 예외가 발생
...
--기본키에 중복된 값을 담으려고 할때 (UNIQUE제약 조건 위반)
INSERT INTO MEMBER
VALUES(NULL, 'USER02', 'PASS02', '홍길순', '여', '010-1111-2222', 'bbb@naver.com', '25/07/30');
--기본키에 중복된 값을 담으려고 할때 (NOT NULL 제약 조건 위반)

INSERT INTO MEMBER
VALUES(3, 'USER03', 'PASS03', '홍길삼', '여', '010-1111-2222', 'bbb3@naver.com', '25/07/30');
INSERT INTO MEMBER
VALUES(4, 'USER04', 'PASS02', '홍길사', '여', '010-1111-2222', 'bbb4@naver.com', '25/07/30');


--복합키 : 두개의 컬럼을 동시에 하나의 주키로 지정하는 것
...
INSERT INTO MEM
VALUES(2, 'USER2', 'PASS1', '홍길이', NULL, NULL, NULL, '25/01/01', 10);
INSERT INTO MEM
VALUES(3, 'USER3', 'PASS1', '홍길삼', NULL, NULL, NULL, '25/01/01', 20);
INSERT INTO MEM
VALUES(4, 'USER4', 'PASS1', '홍길사', NULL, NULL, NULL, '25/01/01', 40);
INSERT INTO MEM
VALUES(5, 'USER5', 'PASS1', '홍길오', NULL, NULL, NULL, '25/01/01', 50);
...
--ㅡMEM_GRADE(쿠모테이블) -> ---| ----MEM(자식테이블)
-- 부모테이블 : 외래키를 통해 부모테이블을 참조하는 테이블
-- 지식테이블 : 외래키를 통해 부모테이블을 참조하는 테이브
--1 대 N 일경우 보통 쪽이 자식테이블

DELETE FROM MEM_GRADE
WHERE DRADE_CODE = 10;
--MEM테이블에서 10이라는 GRADE_ID가 있기 때문에 부모테이블의 GRADE_CODE 10을 가진 데이터 삭제가 안됨.

DELETE FROM MEM_GRADE
WHERE DRADE_CODE = 30;
--MEM테이블에서 참조하고 있지 않은 30의 GRADE_CODE를 가진 데이터는 삭제 가능함

--자식테이블에서 이미 사용하고 있는 값이 있을경우
--부모테이블로 부터 무조건 삭제가 안되는"삭제 제한" 옵션이 기본값으로 설정

-----------------------------------------------------------------------
/*
    자식테이블 생성ㅅ시 외래키 제약조건 부여할 때 삭제옵션을 함께 지정가능
    *삭제 옵션 : 부모테이블의 데이터 삭제시 그 데이터를 잠고핮고 있는 자식테이블의 데이터를 어떻게 처리?
    
    -ON DELETE RESTROCTED(기본값) : 삭제제한옵션, 자식데이터로부터 쓰이는 부모데이터는 삭제가 안됨.
    -ON DELETE SET NULL : 부모데이터 삭제시 해당데이터를 사용하고 있는 자식데이터의 값을 NULL로 변경
    -ON DELETE CASCADE : 부모데이터 삭제시 해당데이터를 사용하고 있느닞 자식데이터도 모두 삭제
*/
...
--GRADE_CODE 20인 데이터를 참조하던 자식데이터들은 전부 UNLL을 가짐
...
--GRADE_CODE 10인 데이터를 참조하던 자식데이터들은 전부 삭제됨
...
CREATE TABLE MEM(
    ...
    CREARE_AT DATE DEFAULT SYSDATE
);
...
VALUES(4, 'USER4',27);
-->선택되지 않은 컬럼에는 기본적으로 NULL이 적용
-->단, 해당컬럼에 DEFAULT이 부여되어있다면 NULL이 아닌 DEFAULT값이 적용
...
GEANT CONNECT, RESOURCE TO C##KH;

ALTER USER C##KH DEFAULT TABLESPACE USERS QUO;
--KH계정으로 전환
-------------------------------------------------------------------------------
--테이블을 복제할 수 있다. 여기서부터 KH계정으로 진행
CREATE TABLE EMPLOYEE_COPY
AS (SELECT * FROM EMPLOYEE);

--CREATE TABLE 테이블명 AS SELECT문
--AS 뒤에오는 SELECT문의 결과를 기반으로 테이블을 생성
...
--EMPLOYEE_COPY 테이블에 외래키 제약조건을 추가(DEPT_ID)
