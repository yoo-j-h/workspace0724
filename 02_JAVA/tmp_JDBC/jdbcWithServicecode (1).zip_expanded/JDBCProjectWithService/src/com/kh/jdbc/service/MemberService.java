...
public class MemberService {
	...
	public int bulkInsertMembers(List<Member> list) {
		Connection conn = getConnection();
		int result = new MemberDao().bulkInsertMembers(list, conn);
		if(result>0) {
			commit(conn);
		}else {
			rollback(conn);
	...
	public int bulkDeleteMembers(String[] userIds) {
		Connection conn = getConnection();
		int result = new MemberDao().bulkDeleteMembers(userIds,conn);
		if(result>0) {
			commit(conn);
		}else {
			rollback(conn);
