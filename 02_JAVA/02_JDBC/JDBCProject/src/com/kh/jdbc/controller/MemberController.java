...
public class MemberController {
	...
	public void selectMemberAll() {
		...
		//조회된 결과에 따라서 사용자가 보게될 화면 출력
		if(list.isEmpty()) {
			new MemberMenu().displayNoDate("회원목록 조회결과가 없습니다.");
		}else {
			new MemberMenu().displayList(list, "Member");
		}
	}
	...
	public void deleteMember(String userId, String userPwd) {
		...
		if(result>0) {
			new MemberMenu().displaySuccess("성공적으로 회원을 삭제했습니다.");
		}else {
