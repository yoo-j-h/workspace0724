...
public class Run {
	/*
	 * 자바 스트림 개념
	 * -프로그램과 외부매체(파일, 네트워트, 콘솔...)간의 데이터 전달 통로
	 * -데이터를 순차적으로 흘려보내는 파이프같은 구조.
	 * 
	 * 스트림의 특징
	 * 1. 단방향 : 입력이면 입력만, 출력이면 출력만 가능 -> 동시에 원하면 각각 만들어줘야한다.
	 * 2. 순차적 처리(FIFO) : 먼저 보낸 데이처가 먼저 나온다.
	 * 3. 시간지연 가능성 : 외부매체와 연결되므로 입출력 속도가 상대적으로 느릴 수 있음.
	 * 
	 * 스트림의 구분
	 * > 데이터 단위에 따른 구분(바이트스트림(1byte), 문자스트림(2byte))
	 * 	 바이터스트림(InputStream, OutputStream) : 이미지, 동영상, 바이너리 데이터등...
