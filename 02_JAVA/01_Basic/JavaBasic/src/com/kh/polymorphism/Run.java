...
public class Run {
	...
	public static void main(String[] args) {
		...
		//cake까지 접근이 가능하며 메모리공간은 실제 cheesecake의 공간까지 존재함.
		c1.sweet();
		//오버라이딩시에는 실제메모리에 있는 재정의된 메서드가 호출됨.
		c1.yummy();
		//다운캐스팅 : 자식의 참조변수로 타입을 변경(업캐스팅 관계에 있을 경우에만 가능)
		((CheeseCake)c1).milky();
		
		
		//CheeseCake c2 = new Cake();
		//실제 메모라가 cake이므로 cheeseCake공간에 접근할 수 없음.
		...
		((Car)c2).drive();
		//((Car)c2).moveAvante(); //업캐스팅시 자식요소의 메모리 접근이 불가함.
